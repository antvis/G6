...
describe('drag signle layer group', () => {
  ...
  it('drag signle layer group', () => {
    ...
    // const cx = (w + 2 * x) / 2;
    // const cy = (h + 2 * y) / 2;
    // expect(keyShape.attr('r')).toEqual(96);
    ...
    // expect(node1.x).toEqual(124.5);
    // expect(node1.y).toEqual(149.5);
    // expect(node1.x).toEqual(124.5);
    // expect(node2.y).toEqual(149.5);

    // // 拖动以后，节点group的matrix值
    // const node = graph.findById(node1.id);
    // const matrix = node.get('group').getMatrix();
    // expect(matrix[6]).toEqual(125);
...
//     expect(graph.destroyed).toBe(undefined);
...
//       expect(groupStyle.r).toEqual(r);
//     }

//     // 指定groupId，验证渲染后的位置是否正确
//     const shape = groupControll.getDeletageGroupById('group2');
//     const shapeStyle = shape.groupStyle;
//     expect(shapeStyle.r).toEqual(31);
//     expect(shapeStyle.x).toEqual(300);
//     expect(shapeStyle.y).toEqual(100);
