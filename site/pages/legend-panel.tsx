...
const LegendPanel: React.FC<IProps> = () => {
  ...
  data.edges.forEach((edge: any) => {
    ...
    const opacity = edge.isReal ? global.edge.style.realEdgeOpacity : global.edge.style.strokeOpacity;
    ...
    return (
      <div id='legend-panel'>
        ...
        <div id='discription-container'>
          ...
          <span className='description'>如果一次聚合后的节点数仍然庞大，可以进行多层次的聚合。为了控制渲染节点的数量，展开多个聚类后，最早被展开的聚类将会被自动收起。这一方案除了满足上述原则，还能减少前端计算和渲染的负担。</span>
          <br /><br />
          <h3 className='legned-title'>{`<图交互>`}</h3>
          <img src='https://gw.alipayobjects.com/mdn/rms_f8c6a0/afts/img/A*IgoxQ7wfjCcAAAAAAAAAAAAAARQnAQ' width='120' />  &nbsp; &nbsp; &nbsp; &nbsp;<img src='https://gw.alipayobjects.com/mdn/rms_f8c6a0/afts/img/A*cCk4SrHVfDsAAAAAAAAAAAAAARQnAQ' width='150' />
          <br /><br />
          <span className='description'> 每个“聚合点” <img src='https://gw.alipayobjects.com/mdn/rms_f8c6a0/afts/img/A*1y4AS7ucVXMAAAAAAAAAAAAAARQnAQ' width='50' />
            代表了一个 LOUVAIN 计算出的聚类，包含多个“真实节点” <img src='https://gw.alipayobjects.com/mdn/rms_f8c6a0/afts/img/A*IOgvSLWF1IQAAAAAAAAAAAAAARQnAQ' width='20' />.
             <strong>「右击」</strong> 任意节点或边，一个相对应的上下文菜单将会出现。
            右击 <img src='https://gw.alipayobjects.com/mdn/rms_f8c6a0/afts/img/A*1y4AS7ucVXMAAAAAAAAAAAAAARQnAQ' width='50' /> 并选择“展开聚合节点”，聚合节点将会被该聚类中的真实节点替代，这就是下钻式探索。
            你也可以通过右击 <img src='https://gw.alipayobjects.com/mdn/rms_f8c6a0/afts/img/A*IOgvSLWF1IQAAAAAAAAAAAAAARQnAQ' width='20' /> 并选择“聚合该聚类”将已经展开的节点聚合；或选择 “寻找 k 度邻居”，被选中点的 k 度邻居节点将会被融合到当前图中。
          </span>
          <br /> <br />
          <h3 className='legned-title'>{`<画布菜单>`}</h3>
          <img src='https://gw.alipayobjects.com/mdn/rms_f8c6a0/afts/img/A*FKbFRIzj34EAAAAAAAAAAAAAARQnAQ' width='300' />
          <br />
          <span>在画布左上角，有一个画布菜单，包含一系列辅助探索的工具。从左到右，它们分别是：</span>
          <br />
          <span><strong>- 显示/隐藏边标签；<br />- 鱼眼放大镜；<br />- 拉索选择模式；<br />- 寻找最短路径（按 SHIFT 点选两个端点）；<br />- 缩小；<br />- 使图内容适应视窗；<br />- 放大；<br />- 搜索一个节点（输入 ID）。</strong></span>

          <br /> <br />
          <h3 className='legned-title'>{`<注意>`}</h3>
          <span>该 demo 仅为展示大规模图可视化方案，因此使用的数据是一个较小的、模拟的数据集。除了上述内容外，还有很多其他的功能。愉快地探索吧。希望它对你有所帮助。</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div id='legend-panel'>
      ...
      <div id='discription-container'>
        ...
        <span className='description'> If the number of nodes still large on aggregated graph, we can do multi-level aggregation. To control the number of rendering nodes, the earliest expanded cluster will be collapsed automatically. These rules also help us to avoid overloaded computation and rendering on front-end.</span>
        <br /><br />
        <h3 className='legned-title'>{`<Graph Interaction>`}</h3>
        <img src='https://gw.alipayobjects.com/mdn/rms_f8c6a0/afts/img/A*IgoxQ7wfjCcAAAAAAAAAAAAAARQnAQ' width='120' />  &nbsp; &nbsp; &nbsp; &nbsp;<img src='https://gw.alipayobjects.com/mdn/rms_f8c6a0/afts/img/A*cCk4SrHVfDsAAAAAAAAAAAAAARQnAQ' width='150' />
        <br /><br />
        <span className='description'>  Each 'Aggregated Node' <img src='https://gw.alipayobjects.com/mdn/rms_f8c6a0/afts/img/A*1y4AS7ucVXMAAAAAAAAAAAAAARQnAQ' width='50' />
          represents a cluster generated by LOUVAIN, it contains several 'Real Node' <img src='https://gw.alipayobjects.com/mdn/rms_f8c6a0/afts/img/A*IOgvSLWF1IQAAAAAAAAAAAAAARQnAQ' width='20' />.
           <strong>「Right Click」</strong> any node or edge on the graph, a corresponding contextmenu will show up.
          Right click <img src='https://gw.alipayobjects.com/mdn/rms_f8c6a0/afts/img/A*1y4AS7ucVXMAAAAAAAAAAAAAARQnAQ' width='50' /> and select 'Expand Node', the aggregated node will be replaced by the real nodes of the cluster.
          You can also right click <img src='https://gw.alipayobjects.com/mdn/rms_f8c6a0/afts/img/A*IOgvSLWF1IQAAAAAAAAAAAAAARQnAQ' width='20' /> and select 'Collapse the Cluster' to collapse it, or select 'Find k-Degree Neighbor', A neighbor graph of the selected node will be merged into the current graph.
        </span>
        <br /> <br />
        ...
        <span>The demo shows a small mocked dataset just for demonstration. Besides the functions introduced above, there are lots of other functions. We hope it is helpful for you. Explore it and have fun!</span>
      </div>
    </div>
  );
};
