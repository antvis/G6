...
describe('brush-select', () => {
  ...
  it('default configs', () => {
    ...
    expect(selectedEdges.length).to.equal(0);
    ...
    expect(selectedNodes[1] === node3).to.be.true;

    selectedEdges = graph.findAllByState('edge', 'selected');
    expect(selectedEdges.length).to.equal(1);
    expect(selectedEdges[0] === edge1).to.be.true;
  ...
  it('modify cfgs', () => {
    ...
    expect(selectedNodes.length).to.equal(2);
    expect(selectedNodes[0] === node1).to.be.true;
    expect(selectedNodes[1] === node2).to.be.true;
