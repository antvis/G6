...
const CALLBACK_URL = 'reference';

const supportedGroups = ['behaviors', 'plugins'];
...
export class MarkdownDocumenter {
  ...
  /**
   * Whether the current page is external of references
   * If `false`, the page will be generated in the `reference` folder
   * If `true`, the page will be generated in the `api` folder
   */
  ...
  private async _writeApiPage(pageData: IPageData) {
    ...
    this._markdownEmitter.emit(stringBuilder, output, {
      contextApiItem: pageData.apiItems[0],
  ...
  private async _writeApiItemPage(apiItem: ApiItem) {
    ...
    const filename: string = path.join(this._outputFolder, CALLBACK_URL, this._getFilenameForApiItem(apiItem));
    const stringBuilder: StringBuilder = new StringBuilder();

    stringBuilder.append('<!-- Do not edit this file. It is automatically generated by G6 API Documenter. -->\n\n');
  ...
  private _getLinkFilenameForApiItem(apiItem: ApiItem): string {
    if (this.isExternal) return '../reference/' + this._getFilenameForApiItem(apiItem);
