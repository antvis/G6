...
| model | Object | true | 元素的数据模型，具体内容参见[元素配置项](/zh/docs/api/Items/item-properties)。 |
...
| --- | --- | --- | --- |
| item | string / Object | true | 元素 ID 或元素实例 |
| model | Object | true | 需要更新的数据模型，具体内容参见[元素配置项](/zh/docs/api/Items/item-properties) |
| stack | boolean | false | 操作是否入 undo & redo 栈，当实例化 Graph 时设置 enableStack 为 true 时，默认情况下会自动入栈，入栈以后，就支持 undo & redo 操作，如果不需要，则设置该参数为 false 即可 |
...
显示指定的元素。若 item 为节点，则相关边也会随之显示。而 [item.show()](/zh/docs/api/Items/item-methods#itemshow) 则将只显示自身。
...
隐藏指定元素。若 item 为节点，则相关边也会随之隐藏。而 [item.hide()](/zh/docs/api/Items/item-methods#itemhide) 则将只隐藏自身。
