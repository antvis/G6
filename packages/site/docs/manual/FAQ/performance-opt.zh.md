...
- 参考官网案例[自定义折线](http://g6.antv.antgroup.com/zh/examples/item/customEdge/#customPolyline)。大部分情况下，折线两个弯折位置分别在两端点（下面例子的`startPoint`、`endPoint`）连线的 1/3 和 2/3 处，其实我们可以轻易计算出简单的折线路径。
```javascript
[
