...
- [Node](/api/element/node/circle)
- [Edge](/api/element/edge/line)
- [Combo](/api/element/combo/circle)
...
5.x 提供了全新的数据 API，详见 [数据 API](/api/graph/method)。
...
G6 5.x 采用了与 4.x 不同的坐标系，详见 [坐标系](/manual/advanced/coordinate)。

<Badge type="error">移除</Badge> **setTextWaterMarker / setImageWaterMarker**

要使用水印功能，请参考 [水印插件](/api/plugin/watermark)。
...
与 G6 4.x 不同，G6 5.x 使用的统一的扩展注册函数(register)，你可以参考 [进阶指南 - 扩展 - 注册](/manual/advanced/extension#注册) 来注册 G6 扩展。
...
完整的事件列表请参考 [事件](/api/export/constants/event)。
