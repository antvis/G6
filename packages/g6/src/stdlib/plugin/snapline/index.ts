...
export default class Snapline extends Base {
  ...
  initAlignLinesForChoose() {
    ...
    allIds.forEach(id => {
      ...
      if(Math.abs(this.draggingBBox.min[1] - cb.center[1]) <= this.tolerance) {
        this.alignLinesForChoose.h.push({
          dl: {
            line: [{x: cb.center[0], y: cb.center[1]}, {x: this.draggingBBox.center[0], y: cb.max[1]}],
            lp: 'top'
          },
  ...
  onDrag(event: IG6GraphEvent) {
    ...
    this.historyPoints[1] = {x: event.canvasX, y: event.canvasY}
   ...
   */
  removeAlignLine([rvdl, rhdl]: [rvdl: boolean, rhdl: boolean]) {
    if (rvdl) {
  ...
  canCancelAbsorb(): [boolean | undefined, boolean | undefined] {
    ...
    if(vdl) {
      if (Math.abs(offset.x) > this.tolerance) {
        ret[0] = true 
      }
    }
    if(hdl) {
      if (Math.abs(offset.y) > this.tolerance) {
        ret[1] = true 
      }
    }
  ...
  updateDraggingItemPosition(dl?:DrawLine, shouldChange?: {x?: number, y?: number}) {
    ...
    function updateNodePosition(lp: LinePosition, diff: number) {
      let x = this.draggingBBox.center[0]
      let y = this.draggingBBox.center[1]
      if (lp === 'left' || lp === 'vcenter' || lp === 'right') {
        x = x - diff 
      }
      if (lp === 'top' || lp === 'hcenter' || lp === 'bottom') {
        y = y - diff 
      }
      
      this.graph.updateNodePosition({id: this.dragItemId, data: {x: x, y: y}})
    }
    ...
    if(dl) {
      ...
      // 先算出diff，然后吸附过去
      if(dl.lp === 'left') {
        diff = this.draggingBBox.min[0] - dl.line[0].x
      }
      if(dl.lp === 'vcenter') {
        diff = this.draggingBBox.center[0] - dl.line[0].x
        updateNodePosition(dl.lp, diff)
      }
      if(dl.lp === 'right') {
        diff = this.draggingBBox.max[0] - dl.line[0].x
        updateNodePosition(dl.lp, diff)
      }
      if(dl.lp === 'top') {
        diff = this.draggingBBox.min[1] - dl.line[0].y
      }
      if(dl.lp === 'hcenter') {
        diff = this.draggingBBox.center[1] - dl.line[0].y
      }
      if(dl.lp === 'bottom') {
        diff = this.draggingBBox.max[1] - dl.line[0].y
      } 
      // 更新draggingBBox位置
