...
export const getPathItem2Card = (item, cardBBox, graph, annotationCanvas) => {
    ...
    if (itemType === 'edge') {
      ...
      const minX = bbox.left,minY = bbox.top, maxX = bbox.right, maxY = bbox.bottom;
      itemLinkPoints = {
    ...
    Object.keys(itemLinkPoints).forEach(key => {
      ...
      itemLinkPoints[key] = annotationCanvas.client2Viewport({ x: clientPos.x, y: clientPos.y});
    });
  
    const { top: cardTop = 0, left: cardLeft = 0, right: cardRight = 0, bottom: cardBottom = 0 } = cardBBox;
    const cardLinkPoints = {
      left: annotationCanvas.client2Viewport({ x: cardLeft, y: (cardTop + cardBottom) / 2 }),
      right: annotationCanvas.client2Viewport({ x: cardRight, y: (cardTop + cardBottom) / 2 }),
      top: annotationCanvas.client2Viewport({ x: (cardLeft + cardRight) / 2, y: cardTop }),
      bottom: annotationCanvas.client2Viewport({ x: (cardLeft + cardRight) / 2, y: cardBottom }),
    };
