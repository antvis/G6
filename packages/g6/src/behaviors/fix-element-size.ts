...
export class FixElementSize extends BaseBehavior {
  static defaultOptions: Partial<FixElementSizeOptions> = {
    enable: (event) => Boolean(event.data.scale! < 1),
    state: 'selected',
    node: undefined,
    edge: [
      {
  ...
  private fixItemSize = async (event: IViewportEvent) => {
    if (!this.validate(event) || !this.isZoomEvent(event)) return;
    ...
    elementData.forEach((elementData) => {
      ...
      const shapes = type === 'edge' ? edge : node;
      ...
      shapeArr.forEach((item: FixShapeConfig) => {
        const { shape: shapeFilter, fields } = item;
        const shape = shapeFilter(getDescendantShapes(el));
  ...
  private resetTransformable = async () => {
  ...
  private bindEvents() {
    ...
    graph.on(GraphEvent.AFTER_TRANSFORM, this.fixItemSize);
    graph.on(GraphEvent.BEFORE_DRAW, this.resetTransformable);
  }

  private unbindEvents() {
    const { graph } = this.context;
    graph.off(GraphEvent.AFTER_TRANSFORM, this.fixItemSize);
    graph.off(GraphEvent.BEFORE_DRAW, this.resetTransformable);
  }
