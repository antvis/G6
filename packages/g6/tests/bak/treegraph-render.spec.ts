...
describe('TreeGraph', () => {
  ...
  it('graph data with tree layout, remove/add/update node, and change layout', (done) => {
    ...
    (async () => {
      await sleep(50);
      await expect(canvas).toMatchSVGSnapshot(dir, 'treegraph-graphdata');
  ...
  it('should be rendered correctly with tree data', (done) => {
    ...
    (async () => {
      await sleep(100);
      await expect(canvas).toMatchSVGSnapshot(dir, 'treegraph-treedata');
      ...
      await sleep(500);
      await expect(canvas).toMatchSVGSnapshot(dir, 'treegraph-treedata-expand');
