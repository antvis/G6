...
export const pluginHull: TestCase = async (context) => {
  ...
  pluginHull.form = (panel) => {
    ...
    return [
      panel
        .add(config, 'hullType', ['convex-hull', 'bubble-sets'])
        .name('Hull Type')
        .onChange((hullType: 'convex-hull' | 'bubble-sets') => {
          updateHullOptions({ hullType });
        }),
      panel
      ...
      panel
        ...
        .onChange((labelCloseToContour: boolean) => {
          updateHullOptions({ labelCloseToContour });
        }),
      panel
