...
- `update` 方法可以不定义：没有指定第三个参数 `extendedNodeName` 时节点更新（`graph.updateItem`，`item.update`）会走 `draw` 方法，所有图形清除重绘；指定了 `extendedNodeName` 时节点更新会走被继承节点类型的 `update` 方法，此时可能会与自定义的 `draw` 方法渲染逻辑不同；
