...
const Body = require('../layout.forceAtlas2/quadTree/body');
const Quad = require('../layout.forceAtlas2/quadTree/quad');
const QuadTree = require('../layout.forceAtlas2/quadTree/quadTree');
...
function forceAtlas2(graph, nodes, edges, params) {
  ...
  do {
    ...
    Forces = getAttrForces(graph, nodes, edges, size, esize, prev_overlapping, dissuade_hubs, mode, iter, prevo_iter, Forces);
    ...
    if (barnes_hut && ((prev_overlapping && iter > prevo_iter) || !prev_overlapping)) {
      Forces = getOptRepGraForces(graph, nodes, edges, size, esize, prev_overlapping, dissuade_hubs, mode, iter, prevo_iter, Forces, kr, kr_prime, kg, center, bodies);
    } else {
      Forces = getRepGraForces(graph, nodes, edges, size, esize, prev_overlapping, dissuade_hubs, mode, iter, prevo_iter, Forces, kr, kr_prime, kg, center);
    }
...
function getAttrForces(graph, nodes, edges, size, esize, prev_overlapping, dissuade_hubs, mode, iter, prevo_iter, Forces) {
  for (let i = 0; i < esize; i += 1) {
...
function getRepGraForces(graph, nodes, edges, size, esize, prev_overlapping, dissuade_hubs, mode, iter, prevo_iter, Forces, kr, kr_prime, kg, center) {
  for (let i = 0; i < size; i += 1) {
    for (let j = i + 1; j < size; j += 1) {
      ...
      let Fr = kr * (nodes[i].degree + 1) * (nodes[j].degree + 1) / eucli_dis; // (0.2 * eucli_dis * eucli_dis);
...
function getOptRepGraForces(graph, nodes, edges, size, esize, prev_overlapping, dissuade_hubs, mode, iter, prevo_iter, Forces, kr, kr_prime, kg, ct, bodies) {
  let minx = 9e10,
    maxx = -9e10,
    miny = 9e10,
    maxy = -9e10;
...
function updatePos(size, nodes, Forces, pre_Forces, SG, ks, ksmax, tao) {
  // the local swg, swgn; the tran
  let swgns = [];
  let trans = [];
  // the global swg, swg(G); the traG
  let swgG = 0;
  let traG = 0;
  for (let i = 0; i < size; i += 1) {
